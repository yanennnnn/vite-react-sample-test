    ...
    <div className='bg-white rounded-lg shadow overflow-hidden'>
      ...
      <div className='p-[24px]'>
          ...
          {todoList.map((item) => 
            <div key={item.id} className='flex items-center py-[4px] border-b border-solid border-neutral-200'>
              <label className="check-box relative">
                <input type="checkbox" id="checkbox" className='invisible'/>
                <span className='mr-[46px]'>{item.content}</span>
              </label>
              <button className='ml-auto focus:outline-none hover:border-transparent group bg-transparent'>
        ...
        <div className='flex justify-between items-center pt-[25px]'>
            <span>{todoList.length ? todoList.filter(item => !item.status).length : 0} 個待完成項目</span>
            <button>刪除全部項目</button>
        </div>
      </div>
    </div>
  )
